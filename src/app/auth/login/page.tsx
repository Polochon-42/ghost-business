export default function LoginPage() {
  const input = {width:'100%',padding:'13px 14px',background:'white',border:'1.5px solid #ddd',borderRadius:'10px',color:'#111',fontSize:'14px',outline:'none',boxSizing:'border-box' as const}
  return (
        ...
        <div style={{background:'white',borderRadius:'16px',padding:'32px',boxShadow:'0 1px 2px rgba(0,0,0,0.08),0 4px 12px rgba(0,0,0,0.05)'}}>
          ...
          <form style={{display:'flex',flexDirection:'column',gap:'16px'}}>
            <div>
              ...
              <input name="email" type="email" placeholder="vous@exemple.com" style={input} />
            </div>
            <div>
            ...
            <button type="submit" style={{width:'100%',padding:'14px',background:'#111',border:'none',borderRadius:'10px',color:'white',fontSize:'15px',fontWeight:'600',cursor:'pointer',letterSpacing:'-0.2px'}}>
              Se connecter
            </button>
          </form>
          <div style={{margin:'20px 0',borderTop:'1px solid #f0f0f0'}}></div>
          <p style={{color:'#717171',fontSize:'14px',textAlign:'center',margin:0}}>
            Pas encore de compte ? <a href="/auth/signup" style={{color:'#111',fontWeight:'600',textDecoration:'underline'}}>Créer un compte</a>
          </p>
        </div>
        <p style={{color:'#aaa',fontSize:'12px',textAlign:'center',marginTop:'20px'}}>Anonymat garanti · Données hébergées au Canada</p>
      </div>
    </div>
  )
}
